...
### Cola de endurecimiento para M6 (triaje de la revisión final de M5)

- **Dashboard sin filtro de `supersedes_id`** (prioridad): `dashboard/page.tsx` agrega el
  historial completo de `risk_results`; el informe 7.9 sí filtra vigentes, así que ambos
  pueden mostrar distribuciones distintas del mismo ciclo en cuanto exista recálculo.
- Decidir supresión complementaria: con una sola celda suprimida, su valor es recuperable
  por resta del total (limitación de la regla n<3 tal como está escrita, no del código).
- Hardening de CSVs contra formula injection (`=`, `+`, `-`, `@`) en `escaparCampoCsv`.
- `individual_result_access` (página individual del RD) inserta a `audit_log` sin revisar
  error; migrarlo a `lib/auditoria.ts`.
- Formatear `generadoEl` como es-MX en el pie del PDF (hoy ISO crudo).
- `acuses-politica.csv`: agregar título/versión de la política acusada.
